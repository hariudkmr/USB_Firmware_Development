/* This file is the part of the Lightweight USB device Stack for STM32
 * microcontrollers
 *
...
#define USB_CLASS_HID \
  0x03 /**<\brief Interface belongs to the Human Interface device class.*/
...
#define USB_HID_REPORT_IN \
  0x00 /**<\brief Indicates that the item is an IN report type.*/
#define USB_HID_REPORT_OUT \
  0x01 /**<\brief Indicates that the item is an OUT report type.*/
#define USB_HID_REPORT_FEATURE \
  0x02 /**<\brief Indicates that the item is a FEATURE report type.*/

/**\name USB HID class-specified requests
 * @{ */
#define USB_HID_GETREPORT \
  0x01 /**<\brief Request to get the current HID report from the device.*/
#define USB_HID_GETIDLE \
  0x02 /**<\brief Request to get the current device idle count.*/
#define USB_HID_GETPROTOCOL \
  0x03 /**<\brief Request to get the current HID report protocol mode.*/
#define USB_HID_SETREPORT \
  0x09 /**<\brief Request to set the current HID report to the device.*/
#define USB_HID_SETIDLE \
  0x0A /**<\brief Request to set the device's idle count.*/
#define USB_HID_SETPROTOCOL \
  0x0B /**<\brief Request to set the current HID report protocol mode.*/
/** @} */

/**\name USB HID class-specified descriptor types
 * @{ */
#define USB_DTYPE_HID 0x21 /**<\brief HID class HID descriptor type.*/
#define USB_DTYPE_HID_REPORT \
  0x22 /**<\brief HID class HID report descriptor type.*/
/** @} */
 ...
 * @{ */
#define HID_PHYSICAL_COLLECTION \
  0x00 /**<\brief A physical collection of items.*/
#define HID_APPLICATION_COLLECTION                                             \
  0x01 /**<\brief Applies a name to a top level collection which the operating \
        ...
        * API.*/
#define HID_LOGICAL_COLLECTION 0x02 /**<\brief A logical collection of \
                                       items.*/
...
#define HID_IOF_VARIABLE \
  (1 << 1) /**<Each field represents data from a physical control.*/
...
#define HID_IOF_RELATIVE                                                      \
  (1 << 2) /**<Data is relative (indicating the change in value from the last \
              report).*/
#define HID_IOF_ABSOLUTE \
  (0 << 2) /**<Data is absolute (based on a fixed origin).*/
#define HID_IOF_WRAP                                                          \
...
#define HID_IOF_NO_WRAP                                                       \
  (0 << 3) /**<Data not “rolls over” when reaching either the extreme high or \
              low value. */
#define HID_IOF_NON_LINEAR \
  (1 << 4) /**<Noninear relationship between logical and physical units.*/
#define HID_IOF_LINEAR \
  (0 << 4) /**<Linear relationship between logical and physical units.*/
#define HID_IOF_NO_PREFERRED_STATE \
  (1 << 5) /**<Control has no preferred state.*/
...
#define HID_IOF_VOLATILE \
  (1 << 7) /**<Data may be changed by device itself. (Output and Feature) */
#define HID_IOF_NON_VOLATILE \
  (0 << 7) /**<Data can be changed by host only. (Output and Feature) */
...
#define HID_COLLECTION(data) \
  HID_RI_COLLECTION(8, data) /**<\copydoc HID_RI_COLLECTION */
#define HID_FEATURE(data) HID_RI_FEATURE(8, data) /**<\copydoc HID_RI_FEATURE \
                                                   */
#define HID_END_COLLECTION \
  HID_RI_END_COLLECTION(0) /**<\copydoc HID_RI_END_COLLECTION */
#define HID_USAGE_PAGE(data)                                                 \
  HID_RI_USAGE_PAGE(8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_USAGE_PAGE \
                              */
#define HID_LOGICAL_MINIMUM(data) \
  HID_RI_LOGICAL_MINIMUM(         \
      8, data) /**<Range [-127; 127]; \copydoc HID_RI_LOGICAL_MINIMUM */
#define HID_LOGICAL_MAXIMUM(data) \
  HID_RI_LOGICAL_MAXIMUM(         \
      8, data) /**<Range [-127; 127]; \copydoc HID_RI_LOGICAL_MAXIMUM */
#define HID_PHYSICAL_MINIMUM(data) \
  HID_RI_PHYSICAL_MINIMUM(         \
      16, data) /**<Range [-32767; 32767]; \copydoc HID_RI_PHYSICAL_MINIMUM */
#define HID_PHYSICAL_MAXIMUM(data) \
  HID_RI_PHYSICAL_MAXIMUM(         \
      16, data) /**<Range [-32767; 32767]; \copydoc HID_RI_PHYSICAL_MAXIMUM */
#define HID_UNIT_EXPONENT(data) \
  HID_RI_UNIT_EXPONENT(         \
      8, data) /**<Range [-127; 127]; \copydoc HID_RI_UNIT_EXPONENT */
#define HID_UNIT(bits, data) HID_RI_UNIT(bits, data) /**<\copydoc HID_RI_UNIT \
                                                      */
#define HID_REPORT_SIZE(data) \
  HID_RI_REPORT_SIZE(8,       \
                     data) /**<Range [0; 0xFF]; \copydoc HID_RI_REPORT_SIZE */
#define HID_REPORT_ID(data) \
  HID_RI_REPORT_ID(8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_REPORT_ID */
#define HID_REPORT_COUNT(data) \
  HID_RI_REPORT_COUNT(         \
      8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_REPORT_COUNT */
#define HID_PUSH HID_RI_PUSH(0) /**<\copydoc HID_RI_PUSH */
#define HID_POP HID_RI_POP(0)   /**<\copydoc HID_RI_POP */
#define HID_USAGE(data) \
  HID_RI_USAGE(8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_USAGE */
#define HID_USAGE_MINIMUM(data) \
  HID_RI_USAGE_MINIMUM(         \
      8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_USAGE_MINIMUM */
#define HID_USAGE_MAXIMUM(data) \
  HID_RI_USAGE_MAXIMUM(         \
      8, data) /**<Range [0; 0xFF]; \copydoc HID_RI_USAGE_MAXIMUM */
//@}
...
#define HID_UNIT_LENGTH(exp) \
  ((exp & 0x0F)              \
   << 4) /**<Length, position, distance unit. cm (CGS), inch (Imperial) */
#define HID_UNIT_ANGLE(exp) \
  ((exp & 0x0F) << 4) /**<Angle unit. radians(CGS), degree (Imperial) */
#define HID_UNIT_MASS(exp) \
  ((exp & 0x0F) << 8) /**<Mass unit. gram (CGS), slug (Imperial) */
#define HID_UNIT_TIME(exp) ((exp & 0x0F) << 12) /**<Time unit. secound. */
#define HID_UNIT_TEMPERATURE(exp) \
  ((exp & 0x0F)                   \
   << 16) /**<Temperature unit. Kelvin (CGS), Fahrenheit (Imperial) */
#define HID_UNIT_CURRENT(exp) ((exp & 0x0F) << 20) /**<Current unit. Ampere */
#define HID_UNIT_LUMINOUS(exp) \
  ((exp & 0x0F) << 24) /**<Luminous intensity unit. Candela */
//@}
...
#define HID_UNIT_NEWTON(exp)                                                 \
  HID_UNIT(16, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_LENGTH(1) | \
                   HID_UNIT_TIME(-2)),                                       \
      HID_UNIT_EXPONENT(exp + 5)
/** SI energy. J = 1E7 * g * cm^2 / s^2 */
#define HID_UNIT_JOULE(exp)                                                  \
  HID_UNIT(16, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_LENGTH(2) | \
                   HID_UNIT_TIME(-2)),                                       \
...
#define HID_UNIT_VOLT(exp)                                                   \
  HID_UNIT(32, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_LENGTH(2) | \
                   HID_UNIT_TIME(-3) | HID_UNIT_CURRENT(-1)),                \
      HID_UNIT_EXPONENT(exp + 7)
/** SI resistance. Ohm = 1E7 * g * cm^2 / (s^3 * A^2) */
#define HID_UNIT_OHM(exp)                                                    \
  HID_UNIT(32, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_LENGTH(2) | \
                   HID_UNIT_TIME(-3) | HID_UNIT_CURRENT(-2)),                \
      HID_UNIT_EXPONENT(exp + 7)
/** SI inductance. H = 1E7 * g * cm^2 / (s^2 * A^2) */
#define HID_UNIT_HENRY(exp)                                                  \
  HID_UNIT(32, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_LENGTH(2) | \
                   HID_UNIT_TIME(-2) | HID_UNIT_CURRENT(-2)),                \
      HID_UNIT_EXPONENT(exp + 7)
/** SI  capacitance. F = 1E-7 * s^4 * A^2 / (cm^2 * g) */
#define HID_UNIT_FARAD(exp)                                                    \
  HID_UNIT(32, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(-1) | HID_UNIT_LENGTH(-2) | \
                   HID_UNIT_TIME(4) | HID_UNIT_CURRENT(2)),                    \
      HID_UNIT_EXPONENT(exp - 7)
...
#define HID_UNIT_TESLA(exp)                                                 \
  HID_UNIT(32, HID_UNIT_CGS_LINEAR | HID_UNIT_MASS(1) | HID_UNIT_TIME(-2) | \
                   HID_UNIT_CURRENT(-1)),                                   \
      HID_UNIT_EXPONENT(exp)
/** SI  illuminance. lx = 1E4 * cd / cm^2 */
#define HID_UNIT_LUX(exp)                                                     \
  HID_UNIT(32,                                                                \
           HID_UNIT_CGS_LINEAR | HID_UNIT_LENGTH(-1) | HID_UNIT_LUMINOUS(1)), \
      HID_UNIT_EXPONENT(exp)
