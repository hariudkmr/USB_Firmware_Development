...
typedef struct {
  uint8_t bmRequestType;  // The transfer direction, the type of request, and
                          // the recipient
  ...
  uint16_t wLength;  // The count of bytes that will be transmitted in the data
                     // data stage
} UsbRequest;
...
/** \name USB standard request
 *
 * @{ */
#define USB_STANDARD_GET_STATUS \
  0x0 /**<\brief Returns the status if the specified recipient */
#define USB_STANDARD_CLEAR_FEATURE \
  0x01 /**<\brief Clears or Disables specific feature */
#define USB_STANDARD_SET_FEATURE \
  0x03 /**<\brief Sets or Enables specific feature */
#define USB_STANDARD_SET_ADDRESS                                              \
  0x05 /**<\brief Sets the device address for all future device communication \
        */
#define USB_STANDARD_GET_DESCRIPTOR \
  0x06 /**<\brief Returns the specified descriptor  */
#define USB_STANDARD_SET_DESCRIPTOR                                           \
  0x07 /**<\brief updates existing descriptor or new descriptor to the device \
        */
#define USB_STANDARD_GET_CONFIG \
  0x08 /**<\brief returns  the current device configuration */
#define USB_STANDARD_SET_CONFIG \
  0x09 /**<\brief sets the device configuration  */
#define USB_STANDARD_GET_INTERFACE 0x0A /**<\brief  */
#define USB_STANDARD_SET_INTERFACE 0x0B /**<\brief  */
#define USB_STANDARD_SYNCH_FRAME 0x0C   /**<\brief  */
/**@}*/
...
#define USB_CLASS_PER_INTERFACE \
  0x00                          /**<\brief Class defined on interface level.*/
...
#define USB_CLASS_HEALTHCARE                                            \
  0x0F                            /**<\brief Personal Healthcare device \
                                     class.*/
...
#define USB_CLASS_IAD \
  0xEF /**<\brief Class defined on interface association level.*/
...
#define USB_SUBCLASS_IAD \
  0x02 /**<\brief Subclass defined on interface association level.*/
...
#define USB_PROTOCOL_IAD \
  0x01 /**<\brief Protocol defined on interface association level.*/
...
typedef struct {
  ...
  uint8_t bDescriptorType; /**<\brief \ref USB_DESCRIPTOR_TYPE_DEVICE device
                              descriptor.*/
  ...
  uint8_t bMaxPacketSize0; /**<\brief Max packet size for the control endpoint 0
                              (8, 16, 32 or 64 bytes).*/
  ...
  uint8_t iManufacturer;   /**<\brief String descriptor index for the
                              manufacturer's name.*/
  uint8_t iProduct; /**<\brief String descriptor index for the product name.*/
  uint8_t iSerialNumber;      /**<\brief String descriptor index for the product
                                 serial number.*/
  uint8_t bNumConfigurations; /**<\brief Total number of configurations
...
typedef struct {
  uint8_t bLength;         /**<\brief Size of the descriptor (in bytes).*/
  uint8_t bDescriptorType; /**<\brief \ref USB_DESCRIPTOR_TYPE_CONFIGURATION
                              descriptor.*/
  uint16_t
      wTotalLength; /**<\brief Size of the configuration descriptor header, and
                       all sub descriptors attached to the configuration.*/
  uint8_t bNumInterfaces;      /**<\brief Total number of interfaces in the
                                  configuration.*/
  uint8_t bConfigurationValue; /**<\brief Configuration index of the current
                                  configuration descriptor.*/
  uint8_t iConfiguration; /**<\brief Index of a string descriptor describing
                             this configuration.*/
  uint8_t bmAttributes;   /**<\brief Configuration attributes: Self Powered and
                             Remote Wake up.*/
...
typedef struct {
  ...
  uint8_t bInterfaceNumber;  /**<\brief Index of the interface in the current
                                configuration. */
  uint8_t bAlternateSetting; /**<\brief Alternate setting for the interface
                                number. */
  uint8_t
      bNumEndpoints; /**<\brief Total number of endpoints in the interface. */
  ...
  uint8_t iInterface; /**<\brief Index of the string descriptor describing the
                         interface. */
...
typedef struct {
  ...
  uint8_t bEndpointAddress; /**<\brief Logical address of the endpoint including
                               direction mask. */
  uint8_t bmAttributes;     /**<\brief Endpoint attributes, \ref
